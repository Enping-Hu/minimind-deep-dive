...
Attention 是 block 里最核心的子层，负责 token 之间的信息交互。这一节把 `Attention.forward`（`model/model_minimind.py`）从头走到尾，重点盯**张量形状**怎么变——看懂形状，就看懂了多头注意力在做什么。
...
同一个 `x` 过三个独立线性层。注意 Q 和 K/V 的输出维度不同：`q_proj` 输出 `8×64=512`，`k_proj`/`v_proj` 只输出 `2×64=128`——因为 KV 头数更少（GQA）。直觉上 Q 是「拿着问题去匹配」，K 是「可被匹配的索引」，V 是「真正被汇聚的内容」。
...
`triu(..., diagonal=1)` 取严格上三角（不含对角线），填 `-inf`：位置 `i` 不能看 `j > i`。关键是那个 `[:, :, :, -seq_len:]` 切片——只对 `scores` 最后一维的**末尾 `seq_len` 列**加掩码。训练时 `scores` 是 `[B, H, T, T]`，末 `seq_len` 列就是全部列，等于整张图加 causal mask。但在 [KV cache](../04-inference/01-kv-cache-and-generate.md) 推理时，K 维含历史 token，`scores` 的最后一维 `> seq_len`（历史 + 当前），此时只对**当前 query 对应的末尾 `seq_len` 列**加上三角——历史那些列当前 query 本就该全部能看，不掩。这个切片让同一行代码同时兼容训练（整图）和增量推理（只掩当前段）。

**2. padding mask 靠两次 unsqueeze 广播到 `[B, H, T, T]`**

```python
extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)   # [B, T] → [B, 1, 1, T]
extended_attention_mask = (1.0 - extended_attention_mask) * -1e9
scores = scores + extended_attention_mask
```

`attention_mask` 是 `[B, T]`（1=有效 token、0=pad）。两次 `unsqueeze(1).unsqueeze(2)` 插出 head 维和 query 维，变 `[B, 1, 1, T]`，加到 `scores [B, H, T, T]` 时广播到所有 head、所有 query 行——即**每个 query 对所有 pad key 列都减 1e9**，softmax 后权重≈0。`(1.0 - mask) * -1e9`：有效位 `(1-1)*-1e9=0` 不变，pad 位 `(1-0)*-1e9=-1e9` 压没。这里用 `-1e9` 而非 causal 的 `-inf`，因为两者会相加，若某行全 `-inf` 会让 softmax 出 NaN，大负数更安全。

**3. `scores.float()` 升精度再 type_as**

```python
scores = F.softmax(scores.float(), dim=-1).type_as(xq)
```

和 [RMSNorm](01-block-and-rmsnorm.md) 同款：softmax 含指数和归一化，对数值精度敏感，低精度训练时先升 float32 算、算完转回原 dtype，避免 `exp` 溢出或精度损失。
...
5.（源码细节）padding mask 为什么要 `unsqueeze(1).unsqueeze(2)`？为什么它用 `-1e9` 而 causal mask 用 `-inf`？
...
5. `attention_mask` 是 `[B, T]`，两次 unsqueeze 插出 head 维和 query 维变 `[B,1,1,T]`，才能广播到 `scores [B,H,T,T]` 的所有 head/query；用 `-1e9` 是因为它和 causal 的 `-inf` 会相加，全 `-inf` 行会让 softmax 出 NaN，大负数更安全。
</details>
