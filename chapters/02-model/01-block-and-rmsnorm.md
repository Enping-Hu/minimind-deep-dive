...
源码：`model/model_minimind.py`，`MiniMindBlock`（L517–553）、`RMSNorm`（L96–121）。

## 模型主链路

`MiniMindModel.forward`（L582–620）把整条链路串起来：

```text
input_ids → embed_tokens → MiniMindBlock × N → 最终 RMSNorm → hidden_states
```

每个 block 输出的 `hidden_states` 是下一个 block 的输入，形状始终是 `[batch, seq_len, hidden_size]`，逐层细化同一组 token 表示。最后还有一个 `self.norm`（L573）做收尾归一化。

## 一个 block 的结构：Pre-Norm + 两次残差

`MiniMindBlock.forward`（L543–553）的核心就六行：
...
block 里有两处 RMSNorm（L538–539），分别在两个子层前。它要解决的问题是：层数一深，`hidden_states` 的数值尺度容易在不同 token、不同层之间漂移，后面的 Attention / FFN 就更难稳定处理。归一化先把尺度收回一个可控范围，再交给子层。

公式（源码 docstring L100–101）：
