...
<details>
<summary>源码细节：[0,1] 空间配对、update 时序、与 GRPO 共用的部分</summary>

正文讲清了 α/β、rho、baseline 映射的作用，这里只补几个正文没点的张量/时序细节（贴真实片段+函数名锚点，无行号，以片段为准）。tracker 代码很直白，不展开背后的统计学；只说清「怎么对齐、什么时候更新」。

**1. reward 也归一到 [0,1] 再喂 α/β——和 baseline 同空间**

正文说 baseline 在 `[0,1]` 空间（`α/(α+β)`），但 α/β 是怎么累加的？`update` 里先把 reward 从 `[-3,3]` 归一到 `[0,1]`，再分别累进 α（"好"的证据）和 β（"差"的证据）：
...
def update(self, rewards, cur_logprobs=None, response_masks=None):
    ...
    self.alpha = rho * self.alpha + avg_normalized_reward         # 归一 reward 累进 α
    self.beta  = rho * self.beta  + (1 - avg_normalized_reward)   # 其补数累进 β
```

所以 `α/(α+β)` 自然落在 `[0,1]`——因为每步往 α 加 `r̄`、往 β 加 `1−r̄`，两者此消彼长。这也解释了正文那个反向映射 `baseline*(2*scale)-scale`：baseline 在 `[0,1]`、reward 在 `[-3,3]`，要先把 baseline 映射回 `[-3,3]` 才能和 raw reward 相减。两处 `scale=3.0` 是配对的。

**2. update 在 backward 之后调用——本步用「旧」baseline，再更新给下一步**

时序容易看错。一个 batch 内的顺序是：`get_baselines`（拿当前 baseline 算 advantage）→ `policy_loss.backward()` → **然后才** `value_tracker.update(...)`：

```python
baselines = value_tracker.get_baselines(len(prompts))   # 本步用的是更新前的 baseline
...
policy_loss = ...; loss.backward()                       # 先反传
rho = value_tracker.update(rewards, per_token_logps.detach(), response_masks)  # 再更新 tracker
```

即**本步的 advantage 用的是上一步留下的 baseline**，本步结束才把这批 reward 并进 tracker、供下一步用。`update` 传的是 `per_token_logps.detach()`——tracker 只拿 log-prob 的数值算 `mean_logprob`（喂给 rho 的 `compute_rho`），不参与梯度，所以 detach。

**3. completion_mask、get_per_token_logps 与 GRPO 同款，只是不分组**

SPO 的 [completion_mask](03-grpo.md)（EOS argmax 三步）和 `get_per_token_logps`（`logits_to_keep=n_keep+1` + shift）和 GRPO 一字不差，区别只在 `generate` 用 `num_return_sequences=1`：每个 prompt 一条回答，张量是 `[B, R]` 而非 GRPO 的 `[B*G, R]`，也就没有 reshape/repeat_interleave 那套组操作。`response_masks = completion_mask.float()` 传给 `update`，让 `mean_logprob` 只统计有效 token。
...
5.（源码细节）`value_tracker.update` 是在 `backward` 之前还是之后调用？这意味着本步 advantage 用的是哪一步的 baseline？
...
5. 在 `backward` 之后。所以本步 advantage 用的是**更新前**（上一步留下）的 baseline，本步结束才把这批 reward 并进 tracker，供下一步用。
</details>
