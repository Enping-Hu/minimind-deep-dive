...
`PretrainDataset.__getitem__`把它变成张量：
...
`SFTDataset.__getitem__`先用 `chat_template` 把对话拼成字符串，再 tokenize，最后构造标签：
...
`DPODataset.__getitem__`对 chosen / rejected 各跑一遍 chat_template + tokenize，再用 `generate_loss_mask` 标出 assistant token，返回错位好的 `x`（输入）、`y`（目标）和 `mask`。怎么用这对偏好算 loss，见第 6 章。
