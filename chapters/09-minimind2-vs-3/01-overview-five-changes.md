...
事实来源：本地直接 diff `minimind-master`（v2）与 `minimind-3`（v3）的 `model/model_minimind.py`、`trainer/`、`tokenizer_config.json`，diff 可复核。
...
E 类里 `hidden_size 512→768` 是**默认值差异，不是 v3 才有的能力**。两版都能配任意 `hidden_size`——v2 默认 512（MiniMind2-Small），v3 默认 768。本书 1–8 章的默认值和参数量都以 v2 默认 512 为准（如 `head_dim=512/8=64`）；到了 v3，默认换成 768，`head_dim` 还能独立配置（A3）。别把「v3 默认更大」读成「只有 v3 能做大模型」。
...
2. `rope_theta`（两版都 1e6）、`tie_word_embeddings`（两版都绑定，v2 硬编码 / v3 config flag）、MoE router 的选择写法（`gate→softmax→topk→norm` 在 v2 就有，不是 v3 新增）。第三项只是「写法同形」：v3 在它之上还把默认 top-k 从 2 降到 1、aux_loss 从序列级换成 batch 级、去掉 shared expert，这些是真变了的。
3. 是默认值差异——两版都能配任意 hidden_size，v2 默认 512、v3 默认 768。区分重要是因为不能把「v3 默认更大」误读成「只有 v3 能做大模型」，本书默认值以 v2 的 512 为准。
</details>
