...
本书正文聚焦 MiniMind 主线：模型结构、预训练、推理、SFT、对齐（DPO/PPO/GRPO/SPO）、训练机制、版本对照、实验。下面四个方向 MiniMind 源码里都有，但属于进阶分支，本书不展开成正文深度，只给最小原理 + 源码/外链入口，供你按需深入。
...
以上四项 + 正文未深入的主题（如 Tokenizer BPE 训练细节、归一化技术综述、GRPO 变体家族 Dr.GRPO/DAPO/GSPO、面试题库）都属于**后续加厚的方向**。本书 v1 忠于 MiniMind 主线源码做完整、准确的讲解，进阶项点到为止——有源码/实操支撑的写深，没有的诚实标注边界，不靠记忆硬扩。
