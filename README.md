...
多数 MiniMind 笔记停在「逐行注释」或「概念讲清」。这份额外做了三件别处少见的事：
...
本仓库随自学进度持续更新。主线（源码精读）已完成，以下按[收录原则](#收录原则)规划：

- **可运行对照实验**（计划中）— `experiments/` 目录放 CPU 可跑的小实验（RoPE 多频证「为什么 d/2 组频率」、Pre/Post-Norm 证「为什么归一化」），把第 10 章的证据扩展到读者可复现。
- **第 10 章实验补充**（更新中）— MiniMind-3 的服务器训练 / 评测仍在进行，后续补更多曲线与评测（含 GRPO 8 层 run id）。
- **RoPE 配图**（待补）— ch02/03 的旋转示意图。

不进主线（仅进阶 / appendix 挂入口）：Flash Attention 的 V1/V2 深入、面试题库。

## 收录原则

一节进入正文主线，须同时满足：① 锚在 MiniMind 真实代码 / 选择上，② 回答源码读不出的「为什么 / 怎么来」。只为对标其他资料、而 MiniMind 未实现的内容不进主线，至多在「进阶」挂入口。

本书是 **v1**：忠于 MiniMind2 主线（默认 `hidden_size=512`，MiniMind2-Small 约 26M）做一遍完整、准确的精读——有源码 / 实操支撑的写深，没有的诚实标边界、点到为止。
